...
export default function Achievements({ db, state, onNavigate }) {
  ...
  return (
    <div className="animate-fade-in">
      <h1 className="text-2xl font-bold tracking-tight text-gray-900 mb-6">🏆 Achievements</h1>

      {/* Level & XP */}
      <div className="card mb-6 ring-0 relative overflow-hidden bg-gradient-to-br from-purple-600 to-indigo-700 text-white shadow-[0_10px_30px_-8px_rgba(79,70,229,0.4)]">
        <div className="absolute -right-6 -top-8 text-8xl opacity-10 select-none" aria-hidden="true">🏆</div>
        <div className="flex justify-between items-start mb-4 relative">
          <div>
            <p className="text-purple-200 text-xs uppercase tracking-wide">Level</p>
            <p className="text-5xl font-black tnums">{gamification.level}</p>
          </div>
          <div className="text-right">
            <p className="text-purple-200 text-xs uppercase tracking-wide">Total XP</p>
            <p className="text-3xl font-bold tnums">{gamification.xp}</p>
          </div>
        </div>
        <div className="relative">
          <div className="flex justify-between text-xs text-purple-200 mb-1.5">
            ...
            <span className="tnums">{levelProgress}%</span>
          </div>
      ...
      <div className="card mb-6">
        ...
        <div className="flex justify-between text-sm text-gray-500 mb-2">
          ...
          <span className="font-bold text-gray-700 tnums">{progressPct}%</span>
        </div>
        <div className="w-full h-4 bg-gray-100 rounded-full overflow-hidden ring-1 ring-gray-900/[0.04]">
          <div
            className="h-full bg-gradient-to-r from-blue-500 via-green-400 via-yellow-400 to-red-500 rounded-full transition-all duration-700 ease-out"
        ...
        <div className="grid grid-cols-2 gap-3">
          {badges.map(badge => {
            ...
            return (
              <div
                key={badge.badge_id}
                className={`p-4 rounded-xl text-center transition-[transform,box-shadow] duration-200 ${
                  earned
                    ? 'bg-amber-50 ring-1 ring-amber-500/20 shadow-sm hover:-translate-y-0.5 hover:shadow-md'
                    : 'bg-gray-50 ring-1 ring-gray-900/[0.04] opacity-50'
                }`}
              >
      ...
      <div className="card mb-6">
        ...
        <div className="space-y-4">
          {quests.map(quest => {
            ...
            return (
              <div key={quest.quest_id} className="ring-1 ring-gray-900/[0.06] rounded-xl p-4">
                <div className="flex justify-between items-center mb-2">
                  <p className="font-semibold text-gray-900">{quest.name}</p>
                  <span className="text-xs text-gray-500 tnums">{progress}/{quest.steps.length}</span>
                </div>
      ...
      <div className="card mb-6">
        ...
        <div className="space-y-2 text-sm">
          {Object.entries(db.gamification.mechanics_catalogue.xp.default_awards).map(([action, xp]) => {
            ...
            return (
              <div key={action} className="flex justify-between items-center p-2.5 bg-gray-50 rounded-lg ring-1 ring-gray-900/[0.03]">
                <span className="text-gray-700">{labels[action] || action}</span>
                <span className="font-bold text-amber-600 tnums">+{xp} XP</span>
              </div>
            )
          })}
        </div>
